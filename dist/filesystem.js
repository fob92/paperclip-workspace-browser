...
export function sanitizeWorkspacePath(pathValue) {
...
export function resolveWorkspacePath(workspacePath, requestedPath = "") {
...
function textSnippet(content, query) {
...
async function resolveExistingEntry(workspacePath, requestedPath = "") {
...
export async function listDirectory(workspacePath, directoryPath = "") {
    ...
    const items = await Promise.all(entries.map(async (entry) => {
        ...
        let sizeBytes = isDirectory ? null : entryStat.size;
        if (isSymlink) {
            try {
...
async function readLeadingBuffer(filePath, limit) {
...
export async function searchWorkspace(workspacePath, query, mode = "both") {
    const safeWorkspacePath = sanitizeWorkspacePath(workspacePath);
    if (!safeWorkspacePath) {
        throw new Error("Workspace has no local filesystem path");
    }
    ...
    async function walk(currentAbsolutePath, currentRelativePath = "") {
        ...
        if (isDirectory) {
            ...
            for (const entry of entries) {
                if (truncated)
                    return;
                if (entry.isDirectory() && SEARCH_IGNORED_DIRECTORY_NAMES.has(entry.name))
                    continue;
        ...
        if (mode === "path")
            return;
        if (stat.size > WORKSPACE_LIMITS.contentSearchBytes)
            return;
        const extension = extensionFor(fileName);
...
export async function buildFileDownload(workspacePath, filePath) {
    const target = await resolveExistingEntry(workspacePath, filePath);
    if (target.stat.isDirectory()) {
...
async function addZipEntry(zip, workspaceRoot, inputPath, zipBasePath, budget, includedPaths) {
...
export async function buildZipDownload(workspacePath, relativePaths, archiveBaseName) {
    const zip = new JSZip();
    const includedPaths = [];
...
export async function buildTerminalCommand(workspacePath, targetPath) {
