...
import { buildFileDownload, buildTerminalCommand, buildZipDownload, listDirectory, readFilePreview, sanitizeWorkspacePath, searchWorkspace, workspaceLabel, } from "./filesystem.js";
...
const plugin = definePlugin({
    async setup(ctx) {
        ...
        ctx.actions.register("search-workspace", async (params) => {
            ...
            return await searchWorkspace(workspace.path, query, mode === "path" || mode === "content" ? mode : "both");
        });
        ...
        ctx.actions.register("download-zip", async (params) => {
            ...
            const requestedPaths = Array.isArray(params.paths)
                ? params.paths.filter((value) => typeof value === "string" && value.trim().length > 0)
                : [];
