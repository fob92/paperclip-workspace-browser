...
function fileTreeNodes(entries: FileEntry[]): FileTreeNode[] {
...
function setChildrenAtPath(nodes: FileTreeNode[], targetPath: string, children: FileTreeNode[]): FileTreeNode[] {
...
export function WorkspaceBrowserPage({ context }: PluginPageProps) {
  ...
  useEffect(() => {
    ...
    hostNavigation.navigate(buildWorkspaceRoute({ projectId: projects[0].id }), { replace: true });
  }, [hostNavigation, projects, query.projectId]);
  ...
  useEffect(() => {
    ...
    hostNavigation.navigate(buildWorkspaceRoute({
      projectId: effectiveProjectId,
      workspaceId: effectiveWorkspace.id,
      file: query.file,
    }), { replace: true });
  }, [effectiveProjectId, effectiveWorkspace, hostNavigation, query.file, query.workspaceId]);
  ...
  async function navigateSelection(next: Partial<QueryState>, replace = false) {
    ...
    hostNavigation.navigate(buildWorkspaceRoute(merged), { replace });
  }
    ...
    >
        ...
        <div style={{ display: "grid", gap: "12px" }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "grid", gap: "4px" }}>
              ...
              <h1 style={{ margin: 0, fontSize: "28px", lineHeight: 1.1 }}>Workspace Files</h1>
      ...
      <section
        style={{
          display: "grid",
          gap: "16px",
          gridTemplateColumns: "minmax(220px, 280px) minmax(260px, 340px) minmax(0, 1fr)",
        }}
      >
        ...
        <section style={sectionStyle}>
          ...
          <FileTree
            ...
            onToggleCheck={(nextPath) => handleToggleCheck(nextPath)}
            ...
            error={rootFileList.error
              ? { message: rootFileList.error instanceof Error ? rootFileList.error.message : String(rootFileList.error) }
              : null}
            empty={{ title: "No files", description: "This workspace does not expose any readable files yet." }}
          ...
          <section style={sectionStyle}>
            ...
            {filePreview.loading ? (
              ...
              <div style={{ borderTop: "1px solid var(--border)", paddingTop: "16px" }}>
                <MarkdownBlock content={filePreview.data.content ?? ""} />
              </div>
